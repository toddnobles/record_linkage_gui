...
def render_viewer(image_ref_col, image_map, columns_to_check):
    ...
    # Select record
    # We make a list of the values in the image ref column of the CSV
    item_list = df[image_ref_col].astype(str).tolist()
    # We provide a dropdown for the user to select which record to view
    selected_item_val = st.selectbox("Select an Item to View", item_list)
    ...
    if selected_item_val:
        ...
        with col_left:
            ...
            if selected_item_val not in st.session_state.all_edits:
                ...
                for col in columns_to_check:
                    st.session_state.all_edits[selected_item_val][col] = str(
                        df.at[row_index, col]
                        )
            ...
            for col in columns_to_check:
                if col not in st.session_state.all_edits[selected_item_val]:
                    st.session_state.all_edits[selected_item_val][col] = (
                        str(df.at[row_index, col])
                    )

            # Show each column as a vertical text input. The alternate here could be to
            # use the st.data_editor but that only displays a horizontal row of a
            # dataframe which isn't as visually clear.
            for col in columns_to_check:
        ...
        # Here we display the image associated with the selected record,
        # and allow user to rotate it
        with col_right:
...
def main():
    ...
    if  (
        ...
        or st.session_state.get("uploaded_file_name") != csv_file.name
    ):
    ...
    render_viewer(
        image_ref_col=image_ref_col,
        image_map=image_map,
        columns_to_check=columns_to_check
        )

# ======= Run App ========
# For now this works with running `streamlit run gui_application.py`.
# We may need to modify this as we package this further
